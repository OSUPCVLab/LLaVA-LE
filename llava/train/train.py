...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="facebook/opt-125m") # opt: open pretrained transformer
...
@dataclass
class DataArguments:
    hf_dataset: str = field(default="pcvlab/lucid",
                            metadata={"help": "HuggingFace dataset repo ID (default: pcvlab/lucid)."})
    hf_config: str = field(default="stage1_captions",
                           metadata={"help": "Dataset config name: stage1_captions | stage2_qa."})
...
def expand2square(pil_img: Image.Image, background_color: tuple) -> Image.Image:
    """Pad a PIL image to a square by centering it on a solid background.

    Args:
        pil_img: Input PIL image (any aspect ratio).
        background_color: RGB tuple used to fill the padding area.
...
class LazySupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning backed by the LUCID HuggingFace dataset.

    Images are returned as pre-decoded PIL objects by the HF dataset, so no
    local image folder is required.  All samples are multimodal (every entry
    has an associated panchromatic image).
    """
    ...
    def __init__(self,
        ...
        rank0_print(f"Loading LUCID dataset from HuggingFace: {hf_dataset} / {hf_config} …")
        self.data = hf_load_dataset(hf_dataset, hf_config, split="train")
        rank0_print(f"Loaded {len(self.data)} samples.")

        # Precompute lengths without decoding images (remove image column for speed)
        text_only = self.data.remove_columns(["image"])
        self._lengths = [
            sum(len(conv["value"].split()) for conv in s["conversations"]) + 128
            for s in text_only
        ]
        # All samples are multimodal → lengths are always positive
        self._modality_lengths = [
            sum(len(conv["value"].split()) for conv in s["conversations"])
            for s in text_only
        ]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        sample = self.data[i]
        # Images arrive as PIL objects from HF; convert to RGB for CLIP
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = LazySupervisedDataset(
        hf_dataset=data_args.hf_dataset,
        hf_config=data_args.hf_config,
        tokenizer=tokenizer,
        data_args=data_args,
    )
...
def train(attn_implementation=None):
    ...
    if training_args.bits in [4, 8]:
        # XXX: quantization of LLM to speed up inference
    ...
    # XXX: vision_tower is openai/clip-vit-large-patch14
